...
type RangeObjectType<
  ...
  End extends number,
  T,
  U extends string | number = RangeUnion<Start, End>,
> = {
  [K in U]: T;
};
...
>(
  ...
  keyMapper?: (i: RangeUnion<Start, End>) => U,
): RangeObjectType<Start, End, T, U> {
  if (start < 0 || end < 0) {
    throw new Error("Range bounds must be non-negative integers");
  }

  const keyMapperFn = keyMapper || ((i) => i as U);

  const result = {} as RangeObjectType<Start, End, T, U>;
