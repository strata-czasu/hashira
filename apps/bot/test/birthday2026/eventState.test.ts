...
describe("Birthday 2026 event state", () => {
  it("distinguishes missing, hidden, disabled, and scheduled events", () => {
    ...
    expect(getBirthday2026EventState({ ...config, visible: false }, start)).toBe(
      "hidden",
    );
    expect(getBirthday2026EventState({ ...config, enabled: false }, start)).toBe(
      "disabled",
    );
    expect(getBirthday2026EventState(config, new Date(start.getTime() - 1))).toBe(
      "not_started",
    );
  });
  ...
  it("calculates seven event days from the 20:00-anchored event window", () => {
    ...
    expect(
      getBirthday2026EventDayIndex(config, new Date(start.getTime() + 86_400_000)),
    ).toBe(1);
