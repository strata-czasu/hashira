...
async function getUserAccesses(prisma: BaseContext["prisma"], guildId: string, userId: string) {
      ...
      .addCommand("user", (command) =>
        command
          ...
          .handle(async ({ prisma }, { user: rawUser }, itx) => {
            ...
            if (dbUser.profileSettings?.tintColorType === "dynamic" && member?.displayColor) {
            ...
            try {
              ...
              if (!(e instanceof DiscordAPIError)) {
                console.error(`Failed to generate user profile image for user ${user.tag}`, e);
              } else {
                console.error(
      ...
      .addGroup("tytuł", (group) =>
        group
          ...
          .addCommand("lista", (command) =>
            command.setDescription("Wyświetl swoje tytuły").handle(async ({ prisma }, _, itx) => {
              if (!itx.inCachedGuild()) return;
          ...
          .addCommand("ustaw", (command) =>
            command
              ...
              .handle(async ({ prisma }, { tytuł: id }, itx) => {
                ...
                if (!ownedTitle) {
                  await itx.editReply("Tytuł o tym ID nie istnieje lub go nie posiadasz!");
                  return;
                }
                const {
          ...
          .addCommand("lista", (command) =>
            command
              ...
              .handle(async ({ prisma }, _, itx) => {
                ...
                const accesses = await getUserAccesses(prisma, itx.guildId, itx.user.id);
            ...
            command
              ...
              .handle(async ({ prisma }, { przedmiot: id }, itx) => {
                ...
                if (!ownedColor?.item.tintColor) {
                  await itx.editReply("Kolor o tym ID nie istnieje lub go nie posiadasz!");
                  return;
                }
                const {
                  item: {
          ...
          .addCommand("z-nicku", (command) =>
            command
              ...
              .handle(async ({ prisma }, _, itx) => {
                ...
                await itx.editReply("Ustawiono dynamiczny kolor profilu z koloru nicku");
              }),
          )
