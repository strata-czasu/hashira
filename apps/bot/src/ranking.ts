...
function formatFooter(total: string | number, periodStart: Date, periodEnd: Date) {
  const parts = [
    typeof total === "string" ? `Razem: ${total}` : `Razem: ${total.toLocaleString("pl-PL")}`,
    formatTimeRange(periodStart, periodEnd),
  ];
  return parts.join(" | ");
}

export const ranking = new Hashira({ name: "ranking" }).use(base).group("ranking", (group) =>
  group
    ...
    .addCommand("user", (command) =>
      command
        ...
        .addString("od", (period) => period.setDescription("Początek danych, np. 2025-01"))
        .addString("do", (period) =>
          period
            .setDescription("Koniec danych, np. 2025-01-15 (domyślnie koniec miesiąca)")
            .setRequired(false),
        )
        .handle(async ({ prisma }, { user, od: rawStart, do: rawEnd }, itx) => {
          if (!itx.inCachedGuild()) return;

          const parsedTimeRange = await parseTimeRange(rawStart, rawEnd, async (message) =>
          ...
          if (!isValidTimeRange(periodStart, periodEnd)) {
            return await errorFollowUp(itx, "Przedział czasowy nie może być dłuższy niż 90 dni.");
          }
          ...
          const paginate = new DatabasePaginator(
            (props, ordering) =>
              prisma.userTextActivity.groupBy({
                ...
                where,
                _count: true,
                orderBy: [{ _count: { channelId: ordering } }, { channelId: ordering }],
              }),
            async () => {
          ...
          const paginator = new PaginatedView(
            ...
            formatEntry,
            true,
            formatFooter(await prisma.userTextActivity.count({ where }), periodStart, periodEnd),
          );
    ...
    .addCommand("kanał", (command) =>
      command
        ...
        .handle(async ({ prisma }, { kanał: channel, od: rawStart, do: rawEnd }, itx) => {
          if (!itx.inCachedGuild()) return;
          ...
          if (channel.type === ChannelType.GuildText) {
            ...
            const formatEntry = (item: PaginatorItem<typeof paginate>, idx: number) => {
              ...
              return `${idx}\\.` + ` <@${item.userId}> - ${formatDuration(totalDuration)}`;
            };
    ...
    .addCommand("serwer", (command) =>
      command
        ...
        .handle(async ({ prisma }, { od: rawStart, do: rawEnd, kanały: rawChannels }, itx) => {
          if (!itx.inCachedGuild()) return;
      ...
      command
        ...
        .handle(async ({ prisma }, { od: rawStart, do: rawEnd }, itx) => {
          ...
          const paginate = new DatabasePaginator(
            (props, ordering) => {
              const sqlOrdering = Prisma.sql([ordering]);
              return prisma.$queryRaw<{ userId: string; total: number; uniqueChannels: number }[]>`
                  select
                    "userId",
                    count(*)::int as "total",
