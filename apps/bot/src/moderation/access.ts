...
const createRestrictionFormatter =
  ({ includeUser, includeChannel }: { includeUser: boolean; includeChannel: boolean }) =>
  (restriction: ChannelRestriction) => {
    ...
    if (restriction.endsAt)
      lines.push(`${bold("Koniec")}: ${time(restriction.endsAt, TimestampStyles.RelativeTime)}`);
...
export const access = new Hashira({ name: "access" }).use(base).group("dostepy", (group) =>
  group
    ...
    .addCommand("odbierz", (command) =>
      command
        ...
        .addString("czas", (czas) => czas.setDescription("Czas blokady").setRequired(false))
        .handle(
          async (
            ...
            const result = await discordTry(
              () => channel.permissionOverwrites.edit(user, { ViewChannel: false }, { reason }),
            ...
            const sentMessage = await sendDirectMessage(
              user,
              composeChannelRestrictionMessage(user, itx.user, channel.name, reason, endsAt),
            );
    ...
    .addCommand("przywroc", (command) =>
      command
        ...
        .addChannel("kanał", (channel) =>
          channel.setDescription("Kanał").setRequired(false).setChannelType(ChannelType.GuildText),
        )
        .handle(
          async (
            { prisma, messageQueue, moderationLog: log },
            ...
            await messageQueue.cancel("channelRestrictionEnd", restriction.id.toString());
    ...
    .addCommand("kanal", (command) =>
      command
        ...
        .addChannel("channel", (ch) =>
          ch.setDescription("Kanał").setRequired(false).setChannelType(ChannelType.GuildText),
        )
