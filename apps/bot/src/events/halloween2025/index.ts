...
const createSpawnComponent = (monster: Halloween2025Monster, spawn: Halloween2025MonsterSpawn) => {
  const lines = [
  ...
  return new ContainerBuilder()
    ...
    .addMediaGalleryComponents((mg) => mg.addItems((mgib) => mgib.setURL(monster.image)))
...
const sendToSpawnChannel = Effect.fn("sendToSpawnChannel")(function* (
  ...
  const guildSettings = getGuildSetting(HALLOWEEN_2025_CHANNELS.SPAWN_CHANNELS, guildId);

  if (!guildSettings)
    return yield* Effect.die(`No halloween2025 channel configured for guild ${guildId}`);

  const channel = client.channels.cache.get(guildSettings) ?? null;

  if (!channel?.isSendable() || channel.isDMBased())
    return yield* Effect.fail(`Channel ${channel?.id} is not sendable in guild ${guildId}`);
...
const sendSpawn = Effect.fn("sendSpawn")(function* (
  ...
  const rarity = yield* Random.choice(["common", "uncommon", "rare", "epic", "legendary"] as const);
...
export const halloween2025 = new Hashira({ name: "halloween2025" })
  ...
  .group("halloween", (group) =>
    group.setDescription("Dowiedz się więcej o Halloween.").addCommand("kolekcja", (command) =>
      command
    ...
    group
      ...
      .addCommand("add-monster", (command) =>
        command
          ...
          .addInteger("weight", (option) => option.setDescription("Waga potwora").setMinValue(1))
          .addString("image-url", (option) => option.setDescription("URL obrazka potwora"))
          .handle(async ({ prisma }, { name, weight, "image-url": image }, itx) => {
            ...
            await itx.reply(`Dodano potwora **${monster.name}** o wadze ${monster.weight}.`);
          }),
      )
      .addCommand("force-spawn", (command) =>
        command
          ...
          .addString("expiration", (option) =>
            option.setDescription("Czas wygaśnięcia spawnu, np. '5m' lub '1h'").setRequired(false),
          )
          ...
          .handle(
            async ({ prisma, messageQueue }, { "monster-id": monsterId, expiration }, itx) => {
              if (!itx.inCachedGuild()) return;
        ...
        command
          ...
          .handle(async ({ prisma }, { role }, itx) => {
            ...
            const results = await modifyMembers(
              members,
              (m) => m.roles.add(role.id, `Dodano rolę uczestnikom eventu Halloween 2025`),
            ...
            await itx.followUp(
              `Dodano rolę ${roleMention(role.id)} ${bold(added.toString())} użytkownikom. ${bold(
