...
const handleStickyMessage = async (
  ...
  const newMessage = await message.channel.send(stickyMessage.content as MessageCreateOptions);
...
export const userTextActivity = new Hashira({ name: "user-text-activity" })
  ...
  .group("user-activity", (group) =>
    group.setDescription("User activity related commands").addCommand("preload", (cmd) =>
      cmd
        ...
        .handle(
          async ({ prisma }, { channel: rawChannel, before: rawBefore, limit: rawLimit }, itx) => {
            ...
            const before =
              rawBefore ?? SnowflakeUtil.generate({ timestamp: Date.now() }).toString();
  ...
  .handle("guildMessageCreate", async ({ stickyMessageCache, userTextActivityQueue }, message) => {
