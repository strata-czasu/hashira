...
export const misc = new Hashira({ name: "strata-misc" }).use(base).group("strata", (group) =>
  group
    .setDefaultMemberPermissions(0)
    .setDescription("Dodatkowe komendy :3")
    .addCommand("ok-lista", (command) =>
      command.setDescription("Zgarnij listę Opiekunów Kanałów").handle(async (_, __, itx) => {
        ...
        const usersById = groupBy(mentionedUsers, (user) => userMention(user.userId));

        const users = mapValues(usersById, (user) => user.map((u) => channelMention(u.channelId)));
        ...
        const paginatedView = new PaginatedView(
          paginator,
          "Opiekunowie Kanałów",
          ([userMention, channels]) => `${bold(userMention)}: ${channels.join(", ")}`,
        );
