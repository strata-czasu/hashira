...
export const colorRoles = new Hashira({ name: "color-role" }).use(base).group("kolorki", (group) =>
  group
    ...
    .addCommand("dodaj", (command) =>
      command
        ...
        .handle(
          async (
            { prisma },
            { nazwa: name, właściciel: owner, wygaśnięcie: rawExpiration, sloty: slots, gradient },
            itx,
          ) => {
            ...
            const expirationText = expiration ? time(expiration) : inlineCode("nigdy");
    ...
    .addCommand("przypisz", (command) =>
      command
        ...
        .addRole("rola", (role) => role.setDescription("Rola do dodania").setRequired(true))
        .addUser("właściciel", (owner) => owner.setDescription("Właściciel koloru"))
        .addInteger("sloty", (slots) => slots.setDescription("Ilość slotów na użytkowników"))

        .addBoolean("gradient", (gradient) =>
          gradient.setDescription("Czy kolor roli może używać gradientów").setRequired(false),
        )
        .handle(
          async ({ prisma }, { rola: role, właściciel: owner, sloty: slots, gradient }, itx) => {
            if (!itx.inCachedGuild()) return;
            if (!itx.memberPermissions.has(PermissionFlagsBits.ManageRoles)) return;
            await itx.deferReply();
      ...
      command
        ...
        .handle(
          async (
            ...
            if (!primaryColor)
              return await errorFollowUp(itx, `Niepoprawny kolor: ${rawPrimaryColor}`);
            ...
            if (rawSecondaryColor) {
              ...
              if (!secondaryColor) {
                return await errorFollowUp(itx, `Niepoprawny kolor: ${rawSecondaryColor}`);
              }
